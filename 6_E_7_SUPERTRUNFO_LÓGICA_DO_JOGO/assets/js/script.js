...
function exibirJogadores() {
  ...
  for (let i = 0; i < jogadores.length; i++) {
    ...
    elemento +=
      "<td class='tabela__nome-time' style='background-image: url(" +
      jogadores[i].imagem +
      ");'>" +
      jogadores[i].nome +
      "</td>";
...
// https://forum.scriptbrasil.com.br/topic/134296-não-permitir-espaço-e-caracteres-especiais-em-campo-do-for/ valida campos com caracteres espciais
function alterarJogador() {
  const nome = document.getElementById("nome").value;
  let nomeNaoValido = document.getElementById("nome-nao-valido");
  if (nome === "" || !nome.trim()) {
    nomeNaoValido.innerHTML = ` O campo esta vazio D: `;
    nomeNaoValido.style.color = "red";
    nomeNaoValido.style.display = "block";
  } else {
    jogadores[0].nome = nome;
    nomeNaoValido.style.display = "none";
  }
  exibirJogadores();
}
...
function sortearCartas() {
  ...
  const nomeRobo = document.getElementById("nome-jogador-robo");
  nomeRobo.style.color = "black";
  nomeRobo.innerHTML = jogadores[1].nome;
...
const nomeUsuario = document.getElementById("nome-jogador-player");
function exibirCarta(jogador) {
  if (jogador == "robo") {
    ...
    for (let i in cartaRobo.atributos) {
      atributos += "<p class='carta__input'>" + i + "</p>";
      atributos +=
        "<p class='carta__atributo'>" + cartaRobo.atributos[i] + "</p>";
    }
    ...
    nomeUsuario.style.color = "black";
    nomeUsuario.innerHTML = jogadores[0].nome;
    ...
    for (let i in cartaUsuario.atributos) {
      atributos +=
        "<input type='radio' name='atributo' value=" + i + " checked>";
      atributos += "<label class='carta__input'>" + i + "</label>";
      atributos +=
        "<p class='carta__atributo'>" + cartaUsuario.atributos[i] + "</p>";
    }
